...
When verdict is FAIL, create a Bug Issue for each failing check vía el CLI `gh` (autenticado como `orquestadoria`). Read `GITHUB_ORG`, `GITHUB_REPO` from `docs/PROJECT.md`.
