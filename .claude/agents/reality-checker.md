...
When the verdict is NEEDS WORK, create a Bug Issue for each critical issue found vía el CLI `gh` (autenticado como `orquestadoria`). Read `GITHUB_ORG`, `GITHUB_REPO` from `docs/PROJECT.md`.
