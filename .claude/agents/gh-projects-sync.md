...
6. Nunca uses tu cuenta personal — siempre `orquestadoria` (token en `.claude/agents/.env`).

---

## ⛔ REGLA DE IDENTIDAD GIT

Como cualquier otro agente, antes de cualquier operación `gh`:

```bash
export GH_TOKEN=$(grep ORCHESTRATORIA_TOKEN .claude/agents/.env | cut -d= -f2)
export GITHUB_TOKEN=$GH_TOKEN
gh auth status 2>&1 | grep -q "orquestadoria" || { echo "❌ Token no es de orquestadoria"; exit 1; }
```
...
|---|---|---|
| `GITHUB_ORG` | Org/usuario propietario del repo | `padelpro-dev` |
| `GITHUB_REPO` | Nombre del repo | `PadelPro` |
| `GITHUB_PROJECT_NUMBER` | Número del Project v2 | `1` |
| `REPO_ROOT` | Ruta absoluta del repo local | `C:/proyectos/PadelPro` |
...
gh project list --owner "$GITHUB_ORG" --format json \
  | jq '.projects[] | select(.title | test("PadelPro"; "i")) | {number, id, title}'
```
...
| Story Points (SP) | Project v2 custom field numérico `Story Points` | Crear si no existe. Update vía GraphQL. |
...
CREATE (X)
- US-008 · Confirmar reserva con OTP Telegram → Issue + Project status:Backlog + label sprint:2
- TICKET-024 · Endpoint POST /reservas → Issue + label sprint:2,priority:must
...
# Setear Story Points (number field)
update_project_sp() {
  local item_id="$1" sp="$2"
  local project_id=$(jq -r '.data.organization.projectV2.id' /tmp/gh-project.json)
  local field_id=$(jq -r '.data.organization.projectV2.fields.nodes[] | select(.name=="Story Points") | .id' /tmp/gh-project.json)
  [[ -z "$field_id" ]] && { echo "⚠️  Project no tiene campo 'Story Points', omitido"; return 0; }
  gh api graphql -f query='
    mutation($p:ID!,$i:ID!,$f:ID!,$v:Float!) {
      updateProjectV2ItemFieldValue(input:{projectId:$p,itemId:$i,fieldId:$f,
        value:{number:$v}}) { projectV2Item { id } }
    }' -F p="$project_id" -F i="$item_id" -F f="$field_id" -F v="$sp"
}
...
- Si un Issue tiene actividad humana (comentarios de personas distintas a `orquestadoria`) y diverge del backlog, **no sobrescribas** el body — añade un comentario explicando la divergencia.
