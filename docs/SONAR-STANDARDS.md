...
- **Nunca uses métodos `@Deprecated`** (S1874). Si un servicio tiene un método
  ...
  proyecto: usar `AuditService.registrar(String, TipoOperacion, …, ResultadoOperacion, String)`
  en lugar del alias `auditService.log(…)` marcado como `@Deprecated`.
...
- Evita cadenas de `instanceof` (S1872); usa polimorfismo o el pattern matching de
  Java 17 (`instanceof Foo f`).

---

## Frontend — React 18 / Vite / MUI v5 (Vitest + Playwright)
...
- **Cobertura mínima obligatoria** (definida en `docs/TESTING-STRATEGY.md`):
  ≥ 80 % de líneas · ≥ 75 % de branches · **100 %** en flujos críticos (triaje,
  AARR, RBAC, incidentes graves, notificación AESIA). El CI bloquea el merge si no
  se cumple.
- Los métodos de test Java deben ser `public`, anotados con `@Test` o
  `@ParameterizedTest`, y con nombre descriptivo en formato
  `debeHacerX_cuandoY()` o `given_when_then`.
