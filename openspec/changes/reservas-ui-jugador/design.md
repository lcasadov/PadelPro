...
El backend añade `creable: boolean` a cada `TramoDisponible` (`true` sii `plazasLibres == max_participants`, equivalente a ocupación 0). La UI ofrece la acción "Reservar" **solo en tramos `creable`**; los parciales (unibles) no se muestran como reservables en v1. Se decidió resolverlo con este flag en vez de exponer `max_participants` o un endpoint de config.
