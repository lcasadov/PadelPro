# Capability: bot-telegram-reservas

## Purpose

Permite a un jugador con cuenta vinculada gestionar sus reservas **desde Telegram** mediante comandos entrantes procesados por el webhook `POST /api/bot/telegram`: crear, listar, confirmar y cancelar reservas. Amplía el dispatcher del bot (que hasta ahora solo atendía `/vincular`) reutilizando los servicios del módulo `reservas`, `OtpService` y `TelegramPort`, sin reimplementar reglas de negocio. Las operaciones críticas (confirmar/cancelar) exigen OTP ligado a la reserva concreta (RN-AUTH-07, D-4).

## Fase

Fase 1 — ✅ Implementada (change `bot-telegram-reservas`, Issue #255, PR #256): dispatcher de comandos en `mensajeria`, `ConfirmarReservaService`, migración `V21__otp_codes_reservation_id.sql` (OTP ligado a reserva).

## Reglas de negocio implicadas

- **RN-TEL-01**: el webhook valida `X-Telegram-Bot-Api-Secret-Token` antes de cualquier parseo; secret ausente/incorrecto → `403`.
- **RN-AUTH-02**: cada operación actúa únicamente sobre reservas del propio usuario autenticado (`USER`).
- **RN-AUTH-07**: OTP de 6 dígitos, TTL 10 min, un solo uso, máx. 3 intentos.
- **RN-RGPD-04**: los logs y la auditoría nunca contienen el OTP en claro.
- Reglas de reserva (anti-solape, precio en backend, plazo de cancelación) provienen de los servicios de `reservas`.

## Requirements

### Requirement: Despacho de comandos entrantes del bot
...
### Requirement: Solo cuentas vinculadas pueden operar
...
### Requirement: Crear reserva con comando estructurado
...
### Requirement: Listar mis reservas con referencia corta
